...
modelfile = "model/vicuna-13b-v1.5-16k.Q4_K_M.gguf"
repetitionpenalty = 1.15
contextlength=16384
logfile = 'Vicuna13b-GGML-promptsPlayground.txt'
print("loading model...")
...
def combine(a, b, c, d):
    ...
    temperature = c
    max_new_tokens = d
    prompt = "USER: " + b + "\n\nASSISTANT:\n"  #removing system message from the prompt
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:   #theme=gr.themes.Glass()
    ...
    with gr.Row(variant='compact'):
            ...
            + "<h1>🦙 Vicuna-13b Q2 16k context window</h1></center>")                    
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            max_len = gr.Slider(label="Maximum output lenght", minimum=10,maximum=12132,step=2, value=1024)
            gr.Markdown(
            """
        ...
        with gr.Column(scale=4):
            txt = gr.Textbox(label="System Prompt", lines=3, interactive=False, value="No system message for this model...")
