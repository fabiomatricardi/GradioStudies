...
def writehistory(text):
    with open('Lamini_PDF_fullEDA_keys.txt', 'a') as f:
        f.write(text)
        f.write('\n')
    f.close()
...
with gr.Blocks(theme=gr.themes.Base()) as demo:   #theme=gr.themes.Glass()
    ...
    with gr.Row():
        ...
        with gr.Column(scale=4):
            gr.Markdown(
            """
            1. Edit the Text extracted from the PDF file
            2. Fill all the metadata
            3. Remove the path from the filename: keep only the name.ext
            4. Click on the button below to SAVE the file and Start Summarization/QnA
            """)            
            rawtext = gr.Textbox(label="extracted text", lines=8,interactive=True,show_copy_button=True )

            btn3 = gr.Button(value="2.🦙 SAVE & Start Summarization", variant='primary')
            txt = gr.Textbox(label="Summary", lines=8,show_copy_button=True)
            gr_keywords = gr.Textbox(label="Keywords", lines=1,value="will be automatically extracted...",interactive=False)
            gr_title = gr.Textbox(label="title", lines=1,interactive=True)
            gr_author = gr.Textbox(label="author", lines=1,interactive=True)
            gr_filename = gr.Textbox(label="original file name", lines=1,interactive=True)
            gr_url = gr.Textbox(label="url", lines=1,interactive=True)
